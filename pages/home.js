  ...
  return (
    <main className="flex min-h-screen items-center justify-center p-6 bg-black">
      <div className="w-full max-w-md">
        <h1 className="text-3xl font-bold text-white mb-8 text-center">Home Page</h1>
        <input
          type="text"
          placeholder="Search..."
          value={searchTerm}
          onChange={handleSearch}
          className="w-full px-4 py-2 mb-4 border border-gray-300 bg-gray-700 text-white rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm"
        />
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          {filteredData.map((name, index) => (
            <div key={index} className="bg-gray-800 rounded-lg shadow-md">
              ...
              {selectedProfile && selectedProfile.bio.includes(name) && (
                <div className="p-4 border-t border-gray-700">
                  <p className="text-gray-400">{selectedProfile.bio}</p>
                </div>
              )}
            </div>
          ))}
